...
function Down_the_matrix(lines, i, index, switch)
	...
	if switch[string.sub(lines[i], index, index)] then
		return 1 + Down_the_matrix(lines, i + 1, index - 1, switch) + Down_the_matrix(lines, i + 1, index + 1, switch)
	else
		return Down_the_matrix(lines, i + 1, index, switch)
	end
end
...
local cont = Down_the_matrix(lines, 1, index, switch)
